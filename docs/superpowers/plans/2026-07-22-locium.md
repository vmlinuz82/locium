...
def test_result_is_normalised():
    assert np.linalg.norm(embed_query("docker compose")) == np.float32(1.0).item() or abs(
        float(np.linalg.norm(embed_query("docker compose"))) - 1.0
    ) < 1e-5
...
def test_find_free_port_returns_the_preferred_one_when_available():
    assert find_free_port(7777) in range(7777, 7800)
