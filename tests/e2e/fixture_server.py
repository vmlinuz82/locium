...
HOME at an isolated directory before importing anything mempalace-related so
tunnel reads/writes never reach the real ``~/.mempalace``. The already-cached
ONNX embedding model is reused via a symlink so search still works offline
instead of trying (and failing) to re-download it under the isolated HOME.
"""
...
if __name__ == "__main__":
    try:
        ...
        build_index(palace, index, tuning=Tuning(dot_cap=DOT_CAP))
